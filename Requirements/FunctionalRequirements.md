...
This is a data-format requirement, specified now so the convention is stable; it is not itself a deploy feature. A definition's four files can be created (an empty scaffold, from the tree — [UC043](UseCases/UC400_ApplicationManagement/UC043_CreateAndDeleteApplicationDefinition.md)), browsed, edited (structured or as combined text), and deleted; actually rendering the templates and submitting the result to Microsoft Graph (the write-back capability referenced above) remains a later phase — see [UC040 — Define an Application](UseCases/UC400_ApplicationManagement/UC040_DefineApplication.md) for the full structure and its remaining open questions.
