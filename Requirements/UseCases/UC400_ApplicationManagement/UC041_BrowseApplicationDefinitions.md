...
6. User selects a file. The extension opens it in the default text editor for that file type (`vscode.open`), exactly as opening any other file in the workspace — directly editable, not a read-only viewer, and not the structured webview from step 3. This remains useful alongside UC042 for direct, low-level edits (e.g. to the Jinja templates' raw syntax) without going through a form.
